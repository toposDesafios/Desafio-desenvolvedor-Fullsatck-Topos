...
app.post("/api/insertEmployee", (request, response) => {
  ...
  (?, ?, ?, ?, ?);`;
  db.run(sql_insert, [request.body.data_nascimento, request.body.nome, request.body.nome_mae, request.body.num_cpf, request.body.num_rg] ,(err, result) => {
    if (err) {
      return console.error(err.message);
    }
...
app.post("/api/insertDependent", (request, response) => {
  const sql_insert = `INSERT INTO Dependente (cod_funcionario, data_nascimento, nome, nome_mae, num_cpf, num_rg) VALUES
  (?, ?, ?, ?, ?, ?);`;
  db.run(sql_insert, [request.body.cod_funcionario, request.body.data_nascimento, request.body.nome, request.body.nome_mae, request.body.num_cpf, request.body.num_rg] ,(err, result) => {
    if (err) {
      return console.error(err.message);
    }
